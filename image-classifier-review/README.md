...
fastai provides `ClassificationInterpretation` for finding the images a model
struggles with most, as well as widget-based tools for cleaning image datasets.
However, I ran into a persistent `ImageClassifierCleaner` widget display issue.

Running the notebook in JupyterLab on Paperspace initially appeared to solve the
problem, following the advice in this
[fast.ai forum thread](https://forums.fast.ai/t/lesson-2-imageclassifiercleaner-error-displaying-widget/109371).
In practice, JupyterLab repeatedly lost its connection, making the review
workflow unreliable.

I built this module as a simple, widget-free alternative for cleaning both
training and validation data. It uses fastai's `ClassificationInterpretation`
to identify high-loss images, displays them in static pages, and applies review
